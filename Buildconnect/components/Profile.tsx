  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-8 py-8 animate-fade-in overflow-x-hidden">
      ...
      {isMe && profile.role !== 'client' && !profile.is_admin && !profile.is_subscribed && (
        <div className="bg-rouge/5 border-2 border-dashed border-rouge p-8 rounded-[2.5rem] mb-12 shadow-inner animate-in zoom-in-95">
          <div className="flex flex-col md:flex-row items-center gap-8 mb-8">
            <div className="flex-1 text-center md:text-left">
              <h3 className="text-2xl font-black text-rouge uppercase mb-2 italic tracking-tighter">Abonnement Pro expiré</h3>
              <p className="text-marron-900 font-bold mb-4">Réactivez votre visibilité maintenant.</p>

              <div className="flex gap-4 justify-center md:justify-start">
                <button onClick={() => setPayUrl("PREMIUM_MONTHLY")} className={`px-4 py-2 rounded-xl text-xs font-black uppercase border-2 ${payUrl.startsWith("PREMIUM_MONTHLY") ? 'bg-rouge text-white border-rouge' : 'border-marron-200 text-marron-400'}`}>
                  Mensuel <br /><span className="text-lg">15.000 F</span>
                </button>
                <button onClick={() => setPayUrl("PREMIUM_YEARLY")} className={`px-4 py-2 rounded-xl text-xs font-black uppercase border-2 ${payUrl.startsWith("PREMIUM_YEARLY") ? 'bg-marron-900 text-white border-marron-900' : 'border-marron-200 text-marron-400'}`}>
                  Annuel <br /><span className="text-lg">150.000 F</span>
                </button>
              </div>
            </div>
            ...
            <div className="w-full md:w-80 space-y-3">
              {profile.has_pending_payment ? <div className="bg-marron-900 text-white p-4 rounded-2xl text-center font-black uppercase text-[10px] animate-pulse">Vérification en cours...</div> :
                <>
                  <p className="text-[10px] font-black uppercase text-marron-400">Payer via Mobile Money au <span className="text-marron-900">+229 0154626444</span></p>
                  <input
                    onChange={e => {
                      const plan = payUrl.split(':')[0] || "PREMIUM_MONTHLY";
                      setPayUrl(`${plan}: ${e.target.value}`);
                    }}
                    placeholder="ID de Transaction (Ex: REF-123456...)"
                    className="w-full p-4 bg-white rounded-xl border outline-none text-xs font-bold focus:ring-2 focus:ring-rouge"
                  />
                  <button onClick={async () => {
                    if (!payUrl.includes(':')) { alert("Veuillez entrer l'ID de transaction"); return; }
                    await fetch(`${API_URL}/users/me/pay`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ screenshot_url: payUrl }) });
                    fetchProfileData();
                  }} className="w-full py-4 bg-rouge text-white rounded-xl font-black uppercase text-[10px] tracking-widest shadow-lg hover:scale-105 transition-transform">
                    Confirmer le dépôt
                  </button>
                </>}
            </div>
          ...
          {profile.projects?.map((p: any) => (
            <div key={p.id} className="bg-white rounded-[2rem] md:rounded-[2.5rem] overflow-hidden shadow-sm hover:shadow-2xl border border-marron-50 group">
              ...
              <div className="p-6 md:p-8">
                <h3 className="font-black text-marron-900 text-lg md:text-xl mb-2">{p.title}</h3>
                <p className="text-marron-400 text-xs md:text-sm leading-relaxed mb-4">{p.description}</p>
                {isMe && (
                  <button
                    onClick={() => {
                      const txn = prompt("Pour booster ce poste (2000 FCFA / 24h), envoyez le montant au +229 0154626444 et entrez l'ID de transaction ici :");
                      if (txn) {
                        fetch(`${API_URL}/users/me/pay`, {
                          method: 'POST',
                          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                          body: JSON.stringify({ screenshot_url: `BOOST_POST_${p.id}: ${txn}` })
                        }).then(() => { alert("Demande de boost envoyée !"); fetchProfileData(); });
                      }
                    }}
                    className="w-full py-2 border-2 border-rouge text-rouge rounded-xl font-black uppercase text-[9px] tracking-widest hover:bg-rouge hover:text-white transition-all"
                  >
                    🚀 Booster ce poste
                  </button>
                )}
